...
interval_l = -1
interval_r =  1
lrands[1] = torch.zeros(100):uniform(interval_l, interval_r)
lrands[2] = torch.zeros(14,14):uniform(interval_l, interval_r)
lrands[3] = torch.zeros(32,32):uniform(interval_l, interval_r)
lrands[4] = torch.zeros(64,64):uniform(interval_l, interval_r)
lrands[5] = torch.zeros(128,128):uniform(interval_l, interval_r)


rrands[1] = torch.zeros(100):uniform(interval_l, interval_r)
rrands[2] = torch.zeros(14,14):uniform(interval_l, interval_r)
rrands[3] = torch.zeros(32,32):uniform(interval_l, interval_r)
rrands[4] = torch.zeros(64,64):uniform(interval_l, interval_r)
rrands[5] = torch.zeros(128,128):uniform(interval_l, interval_r)

rands[1] = torch.zeros(n,100):uniform(interval_l, interval_r)
rands[2] = torch.zeros(n,1,14,14):uniform(interval_l, interval_r)
rands[3] = torch.zeros(n,1,32,32):uniform(interval_l, interval_r)
rands[4] = torch.zeros(n,1,64,64):uniform(interval_l, interval_r)
rands[5] = torch.zeros(n,1,128,128):uniform(interval_l, interval_r)

line  = torch.linspace(0,1,n)

rands[1] = torch.zeros(n,100)
for i=1,n do
   rands[1][i]:copy(lrands[1] * line[i] + rrands[1] * (1- line[i]))
   rands[2][i]:copy(lrands[2] * line[i] + rrands[2] * (1- line[i]))
   rands[3][i]:copy(lrands[3] * line[i] + rrands[3] * (1- line[i]))
   rands[4][i]:copy(lrands[4] * line[i] + rrands[4] * (1- line[i]))
   rands[5][i]:copy(lrands[5] * line[i] + rrands[5] * (1- line[i]))
end
